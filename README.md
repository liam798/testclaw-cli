...
- `CLI`
  - `testclaw` 负责确定性执行、参数标准化，并默认通过 TestClaw Server -> Agent 操作设备
- `SKILL`
...
```bash
TESTCLAW_API=https://testclaw.vvicat.dev testclaw --json whoami
testclaw --api https://testclaw.vvicat.dev --json whoami
```
...
testclaw --json app list-installed --device-id 1
testclaw --json app open --device-id 1 --app-id com.demo.app
testclaw --json suite run --id 12
```

设备业务命令默认不直连运行 CLI 机器上的手机，主路径是：

```text
AI Agent -> testclaw-cli -> TestClaw Server -> TestClaw Agent -> 设备
```

`--adb-address` 仅保留给 `security analyze` 这类显式本地动态分析场景。
...
- `whoami --json` 返回 `base_url`、`auth_mode`、`token_source`、`user`、`roles`、`admin` 和原始后端响应。
- 原始 API 逃生口为：
