...
class SonicBackend {
  ...
  async ensureAuthenticated() {
    ...
    if (!sharedMemHubCredential) {
      throw new SonicApiError("缺少 SonicToken，也没有可复用的 MemHub 登录态。请先执行 memhub auth login 或 testclaw login。");
    }
    try {
      await this.exchangeMemHubCliSession(sharedMemHubCredential);
    } catch (error) {
      throw new SonicApiError(`已检测到 MemHub 登录态，但无法兑换 TestClaw 会话：${error.message}。请确认 TestClaw 后端已部署 /api/oauth/memhub/cli-session，或重新执行 testclaw login。`);
    }
  }

  async exchangeMemHubCliSession(sharedMemHubCredential) {
    const headers = {
      Accept: "application/json",
      "X-API-Key": sharedMemHubCredential.token,
    };
    const response = await fetch(this.buildAbsoluteUrl("/api/oauth/memhub/cli-session"), {
      method: "POST",
      headers,
    });
    const text = await response.text();
    ...
    const sonicToken = session.sonicToken || session.sonic_token || session.token;
    if (!sonicToken) {
      throw new SonicApiError("TestClaw 会话响应缺少 sonicToken。");
    }
    this.config.token = sonicToken;
    this.config.authMode = session.authProvider || session.auth_provider || "memhub";
    this.config.tokenSource = "memhub_exchange";
    this.config.user = {
      id: session.userId ?? session.user_id ?? null,
      username: session.userName || session.username || sharedMemHubCredential.user?.username || null,
      email: session.email || sharedMemHubCredential.user?.email || null,
      name: session.name || session.nickName || session.nickname || session.userName || session.username || null,
    };
    writeTestClawSessionToMemHub(this.config.memhubCredentialsPath, {
  ...
  saveRuntimeConfig() {
    ...
    if (this.config.token && this.config.authMode === "memhub_oidc") {
      writeTestClawSessionToMemHub(this.config.memhubCredentialsPath, {
        baseUrl: this.config.baseUrl,
        authMode: this.config.authMode,
        token: this.config.token,
  ...
  clearAuth() {
    ...
    return {
      ...
      cleared_memhub_testclaw_session: clearedMemHubSession,
  ...
  async doctor({ version } = {}) {
    ...
    pushCheck("auth.permission_probe", !this.config.token || !authenticated || permissionProbe, {
      skipped: !this.config.token || !authenticated,
  ...
  async completeMemHubLogin(result) {
    this.config.token = result.session.sonicToken;
    ...
    this.config.authMode = "memhub_oidc";
    this.config.tokenSource = "memhub_shared";
