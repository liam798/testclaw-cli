...
function extractTcpAddress(value) {
  const text = String(value || "").trim();
  if (!text) {
    return "";
  }
  const adbMatched = /adb\s+connect\s+([^\s]+)/.exec(text);
  if (adbMatched) {
    return adbMatched[1].trim();
  }
  const sibMatched = /sib\s+remote\s+connect\s+--host\s+([^\s]+)\s+-p\s+(\d+)/.exec(text);
  if (sibMatched) {
    return `${sibMatched[1].trim()}:${sibMatched[2].trim()}`;
  }
  return text;
}
...
function shouldPreferDirectCandidate(baseUrl) {
  try {
...
function selectPreferredCandidate(candidates = [], { preferDirect = false } = {}) {
  const available = Array.isArray(candidates)
    ? candidates
        .filter((candidate) => candidate && candidate.available !== false && candidate.url)
        .sort((a, b) => (a.priority || 999) - (b.priority || 999))
    : [];
  const orderedTypes = preferDirect ? ["mesh", "direct", "relay"] : ["mesh", "relay", "direct"];
  for (const type of orderedTypes) {
    const matched = available.find((candidate) => candidate.type === type);
    if (matched) {
      return matched;
    }
  }
...
function buildTcpConnectCommand(service, url) {
  if (!url) {
    return "";
  }
  if (service === "sib") {
    ...
    return `sib remote connect --host ${url.slice(0, separator)} -p ${url.slice(separator + 1)}`;
  }
  return `adb connect ${url.replace(/^adb\s+connect\s+/, "")}`;
}

function resolveConnectionSelection(service, data, baseUrl) {
  const selected = data?.selected?.[service];
  const fallbackCandidate = selectPreferredCandidate(data?.candidates?.[service], {
    preferDirect: shouldPreferDirectCandidate(baseUrl),
  });
  const chosenCandidate = selected?.url ? selected : fallbackCandidate;
  const chosenCommand = chosenCandidate?.url
    ? buildTcpConnectCommand(service, chosenCandidate.url)
    : data?.[service] || data?.legacy?.[service] || "";
  return {
    chosenCandidate,
    chosenCommand,
    address: extractTcpAddress(chosenCommand),
    connectionType: chosenCandidate?.type || (chosenCommand === data?.legacy?.[service] ? "direct" : "legacy"),
    expiresAt: chosenCandidate?.expiresAt || data?.expiresAt || null,
    fallbacks: Array.isArray(data?.candidates?.[service])
      ? data.candidates[service]
          .filter((candidate) => candidate?.url && candidate !== chosenCandidate)
          .map((candidate) => ({
            type: candidate.type,
            url: candidate.url,
            command: buildTcpConnectCommand(service, candidate.url),
          }))
      : [],
  };
}
...
class SonicBackend {
  ...
  async prepareAndroidDebug({ deviceId, udid, sasRemotePort, uia2RemotePort } = {}) {
    ...
    const selection = resolveConnectionSelection("sas", response.data, this.config.baseUrl);
    const adbAddress = selection.address || extractAdbAddress(response.data?.sas);
    return {
      ...response,
      resolvedDevice: {
        id: device.id,
      ...
      adbConnectCommand: selection.chosenCommand,
      adbAddress,
      connectionType: selection.connectionType,
      expiresAt: selection.expiresAt,
      fallbacks: selection.fallbacks,
      adbAddressUsage: selection.connectionType === "direct" ? "same_lan_only" : "recommended",
      executionMode: "server_agent",
      uia2ServerUrl: response.data?.uia2,
      message:
        selection.connectionType === "direct"
          ? "设备已占用，当前返回的是内网直连地址；仅当当前机器与 Agent 处于同网段时可直接 adb connect。"
          : "设备已占用，可直接使用返回的推荐调试命令连接设备。",
    };
  }

  async prepareIosDebug({ deviceId, udid, sibRemotePort, wdaServerRemotePort, wdaMjpegRemotePort } = {}) {
    const device = await this.resolveDevice({ deviceId, udid });
    if (device.platform !== 2) {
      throw new SonicApiError(`设备 ${device.id} 不是 iOS。`);
    }
    if (device.status !== "ONLINE") {
      throw new SonicApiError(`设备 ${device.id} 当前状态为 ${device.status}，无法占用。`);
    }
    const response = await this.request("POST", "/devices/occupy", {
      body: {
        udId: device.udId,
        sibRemotePort: sibRemotePort || 56000 + Number(device.id),
        wdaServerRemotePort: wdaServerRemotePort || 58000 + Number(device.id),
        wdaMjpegRemotePort: wdaMjpegRemotePort || 59000 + Number(device.id),
      },
    });
    const selection = resolveConnectionSelection("sib", response.data, this.config.baseUrl);
    return {
      ...response,
      resolvedDevice: {
        id: device.id,
      ...
      sibConnectCommand: selection.chosenCommand,
      sibAddress: selection.address,
      connectionType: selection.connectionType,
      expiresAt: selection.expiresAt,
      fallbacks: selection.fallbacks,
      wdaServerUrl: response.data?.selected?.wdaServer?.url || response.data?.wdaServer || response.data?.legacy?.wdaServer || "",
      wdaMjpegUrl: response.data?.selected?.wdaMjpeg?.url || response.data?.wdaMjpeg || response.data?.legacy?.wdaMjpeg || "",
      executionMode: "server_agent",
      message:
        selection.connectionType === "direct"
          ? "设备已占用，当前返回的是内网 SIB 连接地址；仅当当前机器与 Agent 处于同网段时可直接连接。"
          : "设备已占用，可直接使用返回的推荐 SIB 连接命令。",
    };
  }
