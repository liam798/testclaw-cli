...
const { installTestClawSkills } = require("./init");
const { oauthLogin } = require("./oauth-client");
const { checkTestClawCliUpdate, updateTestClawCli } = require("./update");
const {
...
命令:
  ...
  case create
  step create
  device list|prepare-android-debug|prepare-ios-debug|release
...
function parseGlobalOptions(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--api") {
      ...
      if (!value) {
        throw new SonicCliError("选项 --api 缺少值。");
      }
      options.api = value;
...
function resolveDeviceSelector(app, options = {}) {
  if (options.adbAddress) {
    throw new SonicCliError("--adb-address 仅用于显式本地动态分析，不再作为 TestClaw 业务设备选择方式；请使用 --device-id 或 --udid。");
  }
  if (options.deviceId || options.udid) {
    return { deviceId: options.deviceId, udid: options.udid };
  }
  if (app.session.selectedDevice) {
    return {
      deviceId: app.session.selectedDevice.deviceId,
      udid: app.session.selectedDevice.udId,
    };
  }
  throw new SonicCliError("缺少 --device-id/--udid，且当前会话没有已准备的设备。");
}
...
async function dispatch(app, argv) {
  ...
  if (command === "bootstrap") {
    const options = parseCommandOptions(commandArgs, {
      ...
      skipUpdate: { flag: "--skip-update", type: "boolean", default: false },
      skipInit: { flag: "--skip-init", type: "boolean", default: false },
    ...
    if (options.help) {
      process.stdout.write(`Usage: testclaw bootstrap
Bootstrap TestClaw CLI for agent skills.

Options:
  --base-url <url>    Persist the target TestClaw Server base URL.
  --skip-update       Skip checking and applying CLI updates.
  --skip-init         Skip syncing testclaw-cli skill files.
  --source-dir <path> Use a local testclaw-skills checkout or testclaw-cli skill directory.
  --target <path>     Install skill only to this skills directory.
  --repo <url>        Override testclaw-skills git repository.
  --ref <name>        Override git branch or tag. Default: main.
...
`);
    ...
    if (!options.skipUpdate) {
      const updateCheck = checkTestClawCliUpdate();
      result.update = updateCheck;
      if (updateCheck.updateAvailable && !options.dryRun) {
        const updateResult = updateTestClawCli({});
        result.update = updateResult;
        result.actions.push("cli_updated");
      } else if (updateCheck.updateAvailable) {
        result.actions.push("cli_update_available");
      } else {
        result.actions.push("cli_up_to_date");
      }
    }

    if (options.baseUrl) {
      const normalizedBaseUrl = writeConfigValue(app, "base_url", options.baseUrl);
      if (!options.dryRun) {
        app.backend.configStore.save(configToPublicKeyValue(app.backend.config));
      }
      result.config = {
        base_url: normalizedBaseUrl,
        saved: !options.dryRun,
      };
      result.actions.push(options.dryRun ? "base_url_checked" : "base_url_saved");
    }

    if (!options.skipInit) {
      const initResult = installTestClawSkills({
      ...
      result.skillInit = initResult;
      result.actions.push(options.dryRun ? "skill_sync_planned" : "skill_synced");
    }
    ...
    process.stdout.write("TestClaw bootstrap completed.\n");
    if (result.update?.updateAvailable) {
      process.stdout.write(`CLI update available: ${result.update.latestVersion}\n`);
    }
    if (result.update?.updated) {
      process.stdout.write("CLI updated.\n");
    }
    if (result.config?.base_url) {
      process.stdout.write(`Base URL: ${result.config.base_url}\n`);
    }
    if (result.skillInit) {
      process.stdout.write("Skill sync completed.\n");
    }
    return;
  }
  if (command === "init") {
    ...
    if (options.help) {
      process.stdout.write(`Usage: testclaw init
Initializes the environment (eg. skills) for TestClaw CLI.

Options:
  --source-dir <path>  Use a local testclaw-skills checkout or testclaw-cli skill directory.
  --target <path>      Install only to this skills directory.
  --repo <url>         Override testclaw-skills git repository.
  --ref <name>         Override git branch or tag. Default: main.
  --dry-run            Print target directories without writing files.
`);
      return;
    }
    const result = installTestClawSkills(options);
    if (app.jsonOutput) {
      emit(app, result);
      return;
    }
    process.stdout.write("Initializing testclaw-cli skill...\n");
    for (const item of result.installations) {
      const verb = result.dryRun ? "would install" : "installed";
      process.stdout.write(`Skill 'testclaw-cli' ${verb} to ${item.installPath}\n`);
    }
    return;
  }
  if (command === "whoami") {
    emit(app, await app.backend.whoami());
    return;
  }
  if (command === "version") {
    emit(app, {
  ...
  if (command === "update") {
    ...
    if (options.help) {
      process.stdout.write(`Usage: testclaw update
Update the TestClaw CLI installed from GitHub.

Options:
  --check          Check the latest installable version without installing.
  --spec <value>   Override npm install spec. Default: git+https://github.com/liam798/testclaw-cli.git
  --verbose        Include npm stdout/stderr in JSON output.
`);
      return;
    }
    const result = updateTestClawCli(options);
    if (app.jsonOutput) {
      emit(app, result);
      return;
    }
    if (options.checkOnly) {
      ...
      process.stdout.write(`Latest: ${result.latestVersion || "unknown"}\n`);
...
默认打开 TestClaw OAuth 授权页，并通过 MemHub 统一身份登录。
`);
      return;
    }
    ...
    emit(app, await app.backend.completeMemHubLogin(result));
    return;
  }
  ...
  if (command === "device" && subcommand === "prepare-ios-debug") {
    const options = parseCommandOptions(subcommandArgs, {
      deviceId: { flag: "--device-id", type: "number" },
      udid: { flag: "--udid" },
      sibRemotePort: { flag: "--sib-remote-port", type: "number" },
      wdaServerRemotePort: { flag: "--wda-server-remote-port", type: "number" },
      wdaMjpegRemotePort: { flag: "--wda-mjpeg-remote-port", type: "number" },
    });
    emit(app, await app.backend.prepareIosDebug(options));
    return;
  }
  ...
  if (command === "app" && subcommand === "list-installed") {
    ...
    const result = await app.backend.listInstalledApps(resolveDeviceSelector(app, options));
  ...
  if (command === "app" && subcommand === "install") {
    ...
    const selector = resolveDeviceSelector(app, options);
    const result = await app.backend.installPackage({ ...selector, packageUrl: options.packageUrl });
  ...
  if (command === "app" && subcommand === "open") {
    ...
    const selector = resolveDeviceSelector(app, options);
    const result = await app.backend.openApp({ ...selector, appId: options.appId });
    app.session.rememberOpenApp(selector, options.appId, result);
    emit(app, result);
    return;
  }
  ...
  if (command === "app" && subcommand === "kill") {
    ...
    const result = await app.backend.killApp({ ...resolveDeviceSelector(app, options), appId: options.appId });
  ...
  if (command === "app" && subcommand === "uninstall") {
    ...
    const result = await app.backend.uninstallApp({ ...resolveDeviceSelector(app, options), appId: options.appId });
...
async function main(argv = process.argv.slice(2)) {
  ...
  if (options.version && !remaining.length) {
    if (options.jsonOutput) {
      ...
      process.stdout.write(`testclaw ${pkg.version}\n`);
    }
    return;
  }
