class CommandSession {
  ...
  rememberOpenApp(selector, appId, payload) {
    const resolvedDevice = payload.resolvedDevice || {};
    this.selectedDevice = {
      deviceId: resolvedDevice.id || selector.deviceId,
      udId: resolvedDevice.udId || selector.udid || selector.udId,
    };
    this.lastResult = payload;
    this.history.push({
      name: "open_app",
  ...
  async undo(app) {
    ...
    if (action.name === "prepare_android_debug") {
      ...
      const udid = action.resolvedDevice.udId || action.selector.udid || action.selector.udId;
      result = await app.backend.killApp({ deviceId: action.resolvedDevice.id || action.selector.deviceId, udid, appId: action.appId });
    } else {
      this.undone.push(action);
      return {
        ok: false,
  ...
  async redo(app) {
    ...
    if (action.name === "open_app") {
      const udid = action.resolvedDevice.udId || action.selector.udid || action.selector.udId;
      const result = await app.backend.openApp({ deviceId: action.resolvedDevice.id || action.selector.deviceId, udid, appId: action.appId });
      this.history.push(action);
      this.lastResult = result;
      return { ok: true, action, result };
    }
